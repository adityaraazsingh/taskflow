...
public class DataInitializer {
    ...
    /** The only demo tenant that receives seed data at startup — every other tenant stays empty. */
    public static final String DEFAULT_TENANT = "DefaultTENANT";

    private final TenantService tenantService;

    public DataInitializer(TenantService tenantService) {
    ...
    CommandLineRunner initData(
        ...
        return args -> {
            // 1. Make sure the demo tenant schema exists (creates schema + runs Liquibase for it).
            String tenantResult = tenantService.createTenant(DEFAULT_TENANT);
            if (tenantResult.startsWith("Error")) {
                log.error("Could not prepare demo tenant '{}' — skipping seed: {}", DEFAULT_TENANT, tenantResult);
                return;
            }

            // 2. Seed data ONLY into the DefaultTENANT demo tenant.
            TenantContext.setTenant(DEFAULT_TENANT);
            try {
                if (userRepo.count() > 0) {
                    log.info("Demo tenant '{}' already contains data — skipping seed.", DEFAULT_TENANT);
                    return;
                }

                log.info("Seeding demo tenant '{}' with initial data...", DEFAULT_TENANT);

                // ══════════════════════════════════════════════
                // 1. USERS  — 10 total
                // ══════════════════════════════════════════════
                UserEntity admin   = createUser("DefaultTENANT/admin",        "Admin@123",    "admin@taskflow.io",     RoleEnum.ADMIN, passwordEncoder);
                UserEntity john    = createUser("DefaultTENANT/johndoe",      "John@123",     "john@taskflow.io",      RoleEnum.USER,  passwordEncoder);
                UserEntity jane    = createUser("DefaultTENANT/janesmith",    "Jane@123",     "jane@taskflow.io",      RoleEnum.USER,  passwordEncoder);
                UserEntity bob     = createUser("DefaultTENANT/bobwilson",    "Bob@123",      "bob@taskflow.io",       RoleEnum.USER,  passwordEncoder);
                UserEntity alice   = createUser("DefaultTENANT/alicemartin",  "Alice@123",    "alice@taskflow.io",     RoleEnum.USER,  passwordEncoder);
                UserEntity chris   = createUser("DefaultTENANT/chrischen",    "Chris@123",    "chris@taskflow.io",     RoleEnum.USER,  passwordEncoder);
                UserEntity diana   = createUser("DefaultTENANT/dianapark",    "Diana@123",    "diana@taskflow.io",     RoleEnum.USER,  passwordEncoder);
                UserEntity ethan   = createUser("DefaultTENANT/ethanlee",     "Ethan@123",    "ethan@taskflow.io",     RoleEnum.USER,  passwordEncoder);
                UserEntity fiona   = createUser("DefaultTENANT/fionawright",  "Fiona@123",    "fiona@taskflow.io",     RoleEnum.USER,  passwordEncoder);
                UserEntity george  = createUser("DefaultTENANT/georgemoore",  "George@123",   "george@taskflow.io",    RoleEnum.USER,  passwordEncoder);
